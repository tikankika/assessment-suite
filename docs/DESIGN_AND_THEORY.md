...
The separation is not complete. Later stages have their required input files declared in code, so an instruction cannot remove a dependency. And when a methodology document cannot be loaded, some stages fall back to guidance written in the code itself. Both are limits on the intention to keep methodological choices in editable documents.
